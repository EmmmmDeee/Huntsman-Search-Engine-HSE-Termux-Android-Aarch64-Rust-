...
This repository already runs an extremely mature, 42-recorded-cycle, evidence-gated issue-remediation practice — root-cause fixes, red-then-green verification, regression-lock tests, explicit REJECTED/DISPROVEN entries with re-verification notes — that is functionally the same discipline this run's directive describes. Re-deriving it from scratch would violate the directive's own "avoid... duplicated implementation" default. Findings at Phase 0 time:

- The ledger's last recorded state (cycle 42) shows **one open item, OD-20** (Low-Medium severity: `ip2location` module has no hosting/datacenter signal, unlike its sibling `criminal_ip`) — explicitly recorded as deferred because closing it needs a live API response check neither that cycle nor this run performed (no `HUNTSMAN_IP2LOCATION_KEY` credential available to exercise it live in this sandbox). Not closed at this point in the run for the same reason: closing it without checking the live response shape would be exactly the "closure claimed without evidence" fabrication this directive treats as its gravest failure. Left OPEN at Phase 0 time, honestly, with the reason recorded.

  **Superseded by PR #550**: the "needs a credential" premise above turned out to be false — the module's own endpoint is keyless at its tier. Re-examined later in this run, called live, and closed as DISPROVEN (a provider-side tier limit, not a code defect). Full evidence chain: `run/ISSUE_LEDGER_PORTABLE.md`'s OD-20 row. The issue ledger now stands at zero open entries.
...
**CLOSED — see PR #552.** The gap recorded at Phase 0 time (below, kept for the historical record) was: `gate.sh` did not regenerate `wasm-ui/pkg/` from source and diff it against the committed copy on every run (the directive's "round-trip drift check"), because doing so needs `wasm-bindgen-cli` and `binaryen` as gate/CI dependencies neither installed. Closed by adding `scripts/wasm_ui_drift_check.sh` (called identically from `scripts/gate.sh` and `.github/workflows/ci.yml`'s `sibling-crates` job) plus a pinned `wasm-bindgen-cli` install (version derived from `wasm-ui/Cargo.toml`, not hardcoded) and a pinned `binaryen` download (exact-version-pinned, not `apt-get install`, because this check's pass/fail is a byte-exact diff that a different `wasm-opt` build could legitimately fail on identical source). Verified both directions before committing: a clean run against the current tree reports no drift, and an injected source change (reverted before committing) was correctly caught and reported.

Original Phase 0 finding, unmodified: `gate.sh` does not currently regenerate `wasm-ui/pkg/` from source and diff it against the committed copy on every run (the directive's "round-trip drift check"). This run manually proved the round-trip once (Artifact Ledger, above) but did not wire it into the automated gate, because doing so requires `wasm-bindgen-cli` and `binaryen` as gate dependencies neither `scripts/gate.sh` nor CI currently installs — a real, scoped, follow-up-worthy addition, recorded honestly as still open rather than silently added without the same verification rigor the rest of this gate already has.
...
## Addendum — later work in this run

This file is a Phase 0 snapshot, intentionally left dated at its original commit (`e67e0c6ba`) rather than rewritten as later work landed — the one exception is the OD-20 correction in §4 above, called out inline as superseded rather than silently edited. PR #548 (offline/vendored build infrastructure), PR #549 (cold-start verification results), and PR #550 (OD-20 closure) all landed after this snapshot; a workspace consolidation pass followed PR #550. See `run/FINAL_REPORT.md` for the run-wide, currently-accurate before/after summary and `run/RUNSTATE.md` for the current at-a-glance status.
