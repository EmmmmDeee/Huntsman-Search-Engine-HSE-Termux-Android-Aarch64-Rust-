...
- **MODE 1 (build from source, offline)**: `cargo build --offline --locked --lib` — succeeded, 0 errors, zero network access. Then `cargo test --offline --locked --lib --bins --tests` against that same fresh extraction ran all 16 test binaries this crate produces (5 `unittests` targets — `src/lib.rs` at 6686 passed/0 failed/22 ignored, plus `hse`, `hse_ai_daemon`, `architecture_audit`, and `gen_oui`'s own bin-level unit tests — and 11 integration-test files under `tests/`: `api`, `architecture`, `audit_regression`, `autonomy_charter`, `cli_seed_validation`, `doc_drift`, `entity_merge_greatest`, `halting`, `install_invariants`, `live_drift`, `smoke`) — **every one reported `0 failed`**.
