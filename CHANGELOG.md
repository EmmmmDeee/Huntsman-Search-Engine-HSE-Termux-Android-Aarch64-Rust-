...
- `core::claim` — the assertion layer the contract's invariants need in order to be machine-checkable. Separates ENTITY / CLAIM / EVIDENCE / INFERENCE; computes a claim lifecycle (Hypothesised → Supported → Corroborated → Established, plus Contested / Refuted / Withdrawn) from the evidentiary record rather than assigning it; counts corroboration by independent source LINEAGE, so one breach corpus resold by three providers is one witness and an inference is never a witness; preserves contradictions with the discriminating evidence that would resolve them instead of collapsing to a winner; models competing hypotheses that refuse premature closure; and scales expansion budget with target difficulty while holding promotion thresholds provably constant (a hard target earns more search, never a lower bar). Pure and fully unit-tested — 22 acceptance tests, each named for the invariant it pins. Not yet consumed by the live scan pipeline.
- `core::geo_confidence` — a geolocation fix that carries its uncertainty: radius, the method that produced it (whose floor the radius can never go below), the observing lineage, and the observation time. Two fixes are never averaged; overlapping ones narrow to the tighter, disjoint ones are preserved as an explicit conflict. Invalid coordinates are refused rather than clamped, and an IP egress or registry service address is marked as an *associated* location, not the subject's own. Not yet emitted by the existing geo modules.
