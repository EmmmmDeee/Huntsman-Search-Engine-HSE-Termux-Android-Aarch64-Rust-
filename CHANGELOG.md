...
### Fixed
- **A provider is no longer disabled for the rest of a scan by a timeout or an echoed number.** The circuit breaker classified an error as a rate limit by looking for bare substrings like `429`, `402`, `exceeded`, or `credit` anywhere in the message — so a transport timeout (`deadline exceeded`), a breach record echoing a phone number that happens to contain `429`/`402`, or the text `credit card` each tripped the long (10-minute) rate-limit cooldown and silently dropped every further result that provider would have returned. Rate-limit detection now uses distinctive quota phrases and matches the `429`/`402` status codes only as standalone tokens, so transient errors and echoed identifiers fall through to the normal three-strike backoff instead of disabling a healthy source.
...
### Added
- **Correlations now carry and are ranked by their MITRE ATT&CK technique breadth.** Each correlation (a cross-source conclusion, e.g. two identifiers that resolve to the same person) now records the distinct ATT&CK Reconnaissance techniques behind it — the union of the techniques that collected its underlying findings — and shows them in the dossier. When two conclusions are otherwise equally ranked, the one corroborated across MORE distinct (independent) techniques is ordered first, so a link confirmed by a breach AND a WHOIS record AND a certificate outranks one re-derived three times by the same kind of source. This makes ATT&CK a measure of source independence, not just per-finding provenance; the confidence-driven primary ordering is unchanged.
### Added
