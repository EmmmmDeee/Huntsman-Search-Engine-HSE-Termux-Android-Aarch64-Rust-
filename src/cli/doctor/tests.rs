...
#[test]
fn validation_label_maps_outcomes_and_never_libels_a_transient_key() {
    assert_eq!(validation_label(Some(true)), "live");
    assert!(validation_label(Some(false)).contains("REJECTED"));
    // An indeterminate probe (unreachable/timeout/429) must read as "unknown",
    // never as a dead key — a transient outage cannot libel a good key.
    let unknown = validation_label(None);
    assert!(unknown.contains("unknown"), "{unknown}");
    assert!(
        !unknown.contains("REJECTED") && !unknown.contains("bad"),
        "{unknown}"
    );
}
...
fn unset_keys_rank_multiplier_first_then_name() {
    ...
    if let (Some(shodan), Some(ip2)) = (pos("HUNTSMAN_SHODAN_KEY"), pos("HUNTSMAN_IP2LOCATION_KEY"))
    {
        assert!(
            shodan < ip2,
