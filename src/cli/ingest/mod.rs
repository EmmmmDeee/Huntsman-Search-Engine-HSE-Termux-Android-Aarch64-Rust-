...
pub async fn run(args: IngestArgs) -> DocumentResult<()> {
    ...
    if args.auto_scan {
        match run_auto_scan(&entities, document_source).await {
            Ok((sid, batch)) => {
                // The shared summary: counted from what was stored, an
...
///
/// Returns the new scan id and what the persist stored
/// ([`PersistedBatch`](crate::app::persist::PersistedBatch): the entity count
/// of the prepared batch it stored, the relation and correlation counts,
/// whether enrichment ran — `false` when skipped for size,
...
async fn run_auto_scan(
    ...
    document_source: &str,
) -> crate::core::error::Result<(String, crate::app::persist::PersistedBatch)> {
...
mod tests {
    ...
    async fn auto_scan_converts_and_persists_the_extracted_batch() {
        ...
        let (sid, batch) = run_auto_scan(&sample(), "notes.txt")
            .await
    ...
    async fn auto_scan_ids_are_unique_across_calls_in_the_same_second() {
        ...
        let (sid_a, _) = run_auto_scan(&sample(), "notes.txt")
            .await
            .expect("first auto-scan persists");
        let (sid_b, _) = run_auto_scan(&sample(), "notes.txt")
            .await
    ...
    async fn auto_scan_drops_placeholder_entities() {
        ...
        let (_sid, batch) = run_auto_scan(&placeholder, "notes.txt")
            .await
