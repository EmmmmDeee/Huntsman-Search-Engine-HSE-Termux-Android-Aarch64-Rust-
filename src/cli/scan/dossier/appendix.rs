...
impl Collection {
    pub(super) fn build(
        ...
        // T2.14: event-sourced hints `analyse()` cannot compute itself (it has
        // no StoragePort access) — a scan-level time+budget signal and a
        // noise-bounded per-module summary. Appended here, with the rest of the
        // computation, so `optimization_hints` is final before the plan reads
        // it to decide whether that appendix exists.
