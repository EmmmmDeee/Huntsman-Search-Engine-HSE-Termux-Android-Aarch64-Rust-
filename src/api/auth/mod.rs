...
//! # Why this exists
//!
//! `hse serve --bind 0.0.0.0:8080` used to expose the whole console to the
//! local network with **no authentication in the binary itself**. That is a
//! materially different exposure from "someone can read my scan results":
//!
...
//! blocks a *browser* from reading a cross-origin response, not a direct
//! client. `routes::enforce_csrf` blocks the drive-by-POST class, but its own
//! doc notes a CLI client simply sends `-H 'X-HSE-CSRF: 1'` — it is a
//! cross-*site* control, not an authentication one. The Host allowlist is
//! applied to loopback binds only.
//!
...
//!
//! A containerised deployment (Railway et al.) layers a *stricter* policy on
//! top at the entrypoint: it refuses to start on a public bind unless
//! `HSE_AUTH_TOKEN` is explicitly set, rather than accepting an auto-minted
//! one. An ephemeral random token is fine for an interactive LAN session an
//! operator starts and reads the log of, but a platform redeploy would mint a
//! *new* one every time and silently lock out anything that saved the old
//! URL — see `docker-entrypoint.sh`.
//!
...
//! The token therefore leaves the address bar (and the browser history, and
//! any `Referer`) after one hop, and every later request — including the
//! SPA's `fetch` calls and the SSE stream, which cannot set headers on
//! `EventSource` — carries the cookie automatically, with no client-side
//! token bookkeeping needed at all. `SameSite=Strict` means a cross-site page
//! cannot ride the cookie, and the existing `X-HSE-CSRF` requirement still
//! applies to mutating methods on top of it.
//!
//! # Comparison is constant-time
//!
//! Tokens are compared as SHA-256 digests through [`ct_eq`], never with `==`
//! on the raw strings. Digesting first makes the compared length constant
//! regardless of what the client sent — both sides are always 32 bytes — so
//! neither the token's length nor its matching prefix is observable through
//! response timing.
...
///
/// Deliberately **not** placed on `AppState`: every handler receives that,
/// and the secret has no business being reachable from request handling. The
/// middleware closure captures this `Arc` the same way the Host allowlist
/// captures its set.
///
/// `Debug` is implemented by hand so the secret cannot reach a log line
...
impl AuthToken {
    ...
    ///
    /// Every other read path must go through [`Self::matches`]. Nothing else
    /// in the tree calls this.
...
///
/// Accumulates the difference of every byte pair with `|=` and compares once
/// at the end, so the loop runs the full length whatever the inputs are and
/// cannot return early on the first mismatching byte. Callers pass SHA-256
/// digests, so both slices are always 32 bytes; the length check is a
/// correctness guard, not a timing-visible branch on secret data.
...
///
/// Reads `/dev/urandom` directly rather than pulling in a RNG crate. That
/// file is present on Linux and on every Android/Termux device HSE targets,
/// it is the kernel CSPRNG (non-blocking after boot, which is long past by
/// the time a user runs `hse serve`), and using it keeps the dependency graph
/// — which must cross-compile to `aarch64-linux-android` — unchanged.
///
...
/// covers the API, the SPA and `/static`. No `Secure`: HSE serves plain HTTP
/// on a LAN or behind a platform's own TLS-terminating proxy (Railway), and
/// marking it `Secure` on a plain-HTTP origin would make the browser drop it
/// outright. No `Max-Age` — it is a session cookie, gone when the browser
/// closes.
...
/// client. The body names the two ways in, without echoing anything the
/// client sent (so a bad token is never reflected into a log or a proxy's
/// history).
...
/// The `/api/v1/health` path stays reachable without a credential so a
/// platform health probe — which is handed no secret — can verify the
/// container. It discloses no scan data.
const AUTH_EXEMPT_PATHS: &[&str] = &["/api/v1/health"];

/// Require a valid token on every request except the exempt health path.
///
/// Installed only for a non-loopback bind. `OPTIONS` is exempt: a CORS
/// preflight carries neither credentials nor side effects, and failing it
/// would surface in the browser as an opaque CORS error instead of the 401
/// that tells the operator what is actually wrong.
///
/// This layer sits on the OUTER app router (nested `/api`, `/static`, the SPA
/// fallback — everything), not just `/api`, so it covers the whole console
/// uniformly rather than leaving the static bundle and SPA shell reachable
/// unauthenticated.
pub async fn enforce_auth(
    ...
    // A valid `?t=` that did NOT come from a header or cookie is the
    // browser's first navigation: strip it from the URL and pin the cookie
    // instead.
