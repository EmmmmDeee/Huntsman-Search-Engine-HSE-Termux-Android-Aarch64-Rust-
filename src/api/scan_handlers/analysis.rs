...
use super::super::handlers::{bad_request, offload, ok_list, ok_paginated_list};
...
pub async fn scan_entities(
    ...
    let id2 = id.clone();
    let mut entities = match offload("query", move || store.entities_for_scan(&id2)).await {
        Ok(v) => v,
...
pub async fn scan_exposure(
    ...
    // async runtime.
    let (mut entities, correlations) = match offload("query", move || {
...
pub async fn scan_diff(
    ...
    let (baseline, later) = match offload("query", move || {
...
pub async fn scan_entities_filter(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let mut entities = match offload("query", move || {
...
pub async fn scan_entities_facets(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let facets = match offload("query", move || store.entity_facets(&id2)).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
...
pub async fn scan_stealer_rows(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let rows = match offload("query", move || store.stealer_rows_for_scan(&id2)).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
    ok_list("rows", rows)
...
pub async fn scan_correlations(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let corr = match offload("query", move || store.correlations_for_scan(&id2)).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
...
pub async fn scan_location(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let (correlations, entities) = match offload("query", move || {
        Ok::<_, crate::core::error::Error>((
...
pub async fn scan_relations(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let (rels, ents) = match offload("query", move || {
        Ok::<_, crate::core::error::Error>((
...
pub async fn scan_identities(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let corefs = match offload("query", move || {
...
pub async fn scan_cross_scan(
    ...
    let (category, min_tier) = match offload("query", move || {
