...
use super::handlers::{not_found, offload};
...
pub async fn scan_export_gexf(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    let (mut entities, relations) = match offload("query", move || {
...
pub async fn scan_debug_bundle(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    match offload("debug-bundle render", move || {
...
pub async fn scan_events_log(
    ...
    let store = std::sync::Arc::clone(&s.store);
    let id2 = id.clone();
    match offload("events-log query", move || store.events_for_scan(&id2)).await {
