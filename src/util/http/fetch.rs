//! HTTP fetch helpers, body reading, and JSON decode for the plain
//! (non-keyed) `fetch_json*` family — plus the low-level primitives
//! ([`breaker_gate`], [`record_breaker_outcome`], [`decode_json_body`]) that
//! [`super::cascade`]'s keyed-cascade retry machinery also shares.
...
/// `fetch_json_inner` and [`super::cascade::fetch_keyed_json`] previously carried
/// verbatim.
pub(super) fn breaker_gate(module: &str, url: &str) -> Result<Option<String>> {
...
/// for a host-less URL.
pub(super) fn record_breaker_outcome(host: Option<&str>, status: reqwest::StatusCode) {
    if let Some(h) = host {
...
/// shared success tail of the JSON fetch helpers, plus [`super::cascade`]'s keyed
/// cascade drivers.
pub(super) async fn decode_json_body<T: DeserializeOwned>(
