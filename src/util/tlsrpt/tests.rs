...
fn parse_mailto_rua_yields_email() {
    ...
    let r = parse("v=TLSRPTv1;rua=mailto:sts-reports@google.com").unwrap();
...
fn parse_https_rua_yields_url() {
    ...
    let r = parse("v=TLSRPTv1; rua=https://tlsrpt.azurewebsites.net/report").unwrap();
...
fn parse_skips_malformed_mailto() {
    ...
    let r = parse("v=TLSRPTv1; rua=mailto:bogus,mailto:ok@ex.com").unwrap();
