...
pub const FEATURE_TOGGLES: &[(&str, bool)] = &[
    ...
    // Expansion redshift: discount an entity's effective confidence FOR
    ...
    // strongly corroborated to keep inflating — a "redshift horizon" on the
    // entity universe. Default **OFF** (byte-identical expansion to today); the
    // raw c_effective every correlation/display/gate reads is never changed.
    // Turn on (`hse config feature.redshift on`) for a tighter, seed-focused
    // sweep that spends its budget nearer the subject.
    ("feature.redshift", false),
];
...
/// The `feature.*` key gating expansion redshift — one source of the key string
/// so the engine gate and the toggle registry can't drift.
pub const REDSHIFT_FEATURE: &str = "feature.redshift";
