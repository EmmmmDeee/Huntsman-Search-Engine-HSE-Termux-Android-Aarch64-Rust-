...
impl QuotaBudget {
    ...
    pub fn operator_pinned_scan_cap(&self) -> bool {
        // Read the current scan's override from `per_scan`, mirroring
        // `scan_cap`'s own read exactly (the override moved off a top-level
        // atomic into per-scan `ScanState`); `0` means "unset", not "cap at 0".
        let scan = current_scan();
