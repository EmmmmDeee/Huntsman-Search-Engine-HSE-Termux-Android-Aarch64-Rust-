...
fn parse_basic_reject_policy() {
    let r = parse("v=DMARC1; p=reject; rua=mailto:dmarc@example.com").expect("should succeed");
...
fn parse_quarantine_with_pct() {
    let r = parse("v=DMARC1; p=quarantine; pct=50; rua=mailto:rep@x.com").expect("should succeed");
...
fn parse_none_policy() {
    let r = parse("v=DMARC1; p=none").expect("should succeed");
...
fn parse_strict_alignment() {
    let r = parse("v=DMARC1; p=reject; adkim=s; aspf=s").expect("should succeed");
...
fn parse_subdomain_policy_override() {
    let r = parse("v=DMARC1; p=reject; sp=none; rua=mailto:a@b.com").expect("should succeed");
    assert_eq!(r.policy, Some(DmarcPolicy::Reject));
...
fn parse_size_suffix_stripped() {
    ...
    let r = parse("v=DMARC1; p=none; rua=mailto:rep@x.com!10m,mailto:rep2@y.com!500k").expect("should succeed");
    assert_eq!(r.rua, vec!["rep@x.com", "rep2@y.com"]);
}

#[test]
fn parse_ri_and_fo_fields() {
    let r = parse("v=DMARC1; p=reject; ri=3600; fo=1").expect("should succeed");
    assert_eq!(r.ri, 3600);
...
fn parse_unknown_tags_ignored() {
    ...
    let r = parse("v=DMARC1; p=reject; newtagfuture=somevalue; rua=mailto:a@b.com").expect("should succeed");
    assert_eq!(r.policy, Some(DmarcPolicy::Reject));
...
fn parse_first_occurrence_wins_for_duplicate_tags() {
    ...
    let r = parse("v=DMARC1; p=reject; p=none").expect("should succeed");
    assert_eq!(r.policy, Some(DmarcPolicy::Reject));
}
...
fn parse_missing_policy_tag_returns_record_with_none_policy() {
    ...
    let r = parse("v=DMARC1; rua=mailto:a@b.com").expect("should succeed");
...
fn issues_partial_coverage() {
    let r = parse("v=DMARC1; p=reject; pct=75; rua=mailto:a@b.com").expect("should succeed");
    let issues = r.issues();
...
fn issues_no_aggregate_reports() {
    let r = parse("v=DMARC1; p=reject").expect("should succeed");
