//! Infrastructure relation builders: the DNS/WHOIS/hosting graph
//! (`SubdomainOf`, `BelongsToDomain`, `HostedOn`, `CoLocatedWith`,
//! `ResolvesTo`, `RegisteredBy`, `DerivedFrom`, `SameOperator`). Every edge
//! here is structural or evidence-grounded — no damped candidate signal lives
//! in this family (those start at [`super::identity`]).
...
pub fn derive_name_lineage(entities: &[Entity], scan_id: &str) -> Vec<Relation> {
    ...
    let persons = super::persons_by_name(entities);
    if persons.is_empty() {
        return Vec::new();
    }
