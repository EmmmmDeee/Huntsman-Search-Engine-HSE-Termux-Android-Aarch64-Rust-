...
fn au032_excludes_candidate_endpoints_not_in_confirmed_slice() {
    ...
    assert!(
        rule_au_032_colocation_cluster(&RuleContext::new(&[c1, c2]), &rels, "s", 0).is_empty(),
...
/// A breach-record-shaped evidence item — `oathnet_pro`/`see_know`'s real
/// (dbname, email, username) record identifier — so AU-039 tests exercise the
/// actual precise [`SourceRecord`] matching, not a placeholder source name with
...
fn au_039_fires_only_for_same_breach_record() {
    // Both wallet and identity from same oathnet_pro record (same dbname + email +
    // username) → should fire with HIGH severity.
    let person = mk_breach_entity(
        EntityKind::Person,
        "Alice Smith",
        "linkedin_2020",
        "alice@example.com",
    ...
    let result = rule_au_039_wallet_identity(&RuleContext::new(&[person, wallet]), "scan", 0);
...
fn au_039_does_not_fire_without_identity() {
    ...
    let result = rule_au_039_wallet_identity(&RuleContext::new(&[wallet]), "scan", 0);
    assert_eq!(result.len(), 0, "wallet without identity should not fire");
}

#[test]
fn au_039_links_wallet_to_source_related_identity() {
    // Genuine co-location: the SAME breach record surfaced both the wallet and the
    // account owner (same dbname + email + username) — a real attribution lead the
    // rule reports.
    let ents = vec![
        mk_breach_entity(
            EntityKind::CryptoAddress,
    ...
    // Co-existence WITHOUT a shared record is not attribution (T2.39): a wallet
    // from a chain module and a person from a disjoint breach record co-occur in
...
#[test]
fn au_039_does_not_attribute_wallet_to_unrelated_identity_in_a_different_record() {
    ...
    // the wallet + one person share the SAME breach record; a second, unrelated
    // person comes from a DIFFERENT record. We deliberately give the UNRELATED
    // person the smaller UID, so the buggy min-UID pick would name them — the fix
    // must instead pick the record-related person and never the bystander.
    ...
    // Smaller-UID person: UNRELATED (a different breach record, would win the buggy pick).
    let unrelated = mk_breach_entity(
        EntityKind::Person,
        &small_uid_name,
        "unrelated_dump",
        "someone@other.com",
    ...
    // Larger-UID person: shares the wallet's exact record ⇒ the genuine attribution.
    let related = mk_breach_entity(
        EntityKind::Person,
        &large_uid_name,
        "stealer_log_1",
        "target@example.com",
    ...
    assert_eq!(
        out.len(),
        1,
        "only the record-related identity is attributed"
    );
    assert!(out[0].entity_uids.contains(&wallet.uid));
    assert!(
        out[0].entity_uids.contains(&related.uid),
        "attributed to the shared-record person"
    );
    assert!(
...
fn au_039_prefers_tied_person_over_email_and_reports_each_tie() {
    // The same breach record surfaced the wallet, two people, and an email. Person
    // is the more specific identity, so both tied people are reported (each an
    // independent, genuine lead) and the redundant email is suppressed.
    let dbname = "stealer_log_1";
    let email_val = "shared@example.com";
    let username = "shared.user";
    let wallet = mk_breach_entity(
        EntityKind::CryptoAddress,
        "1A1zP1eP...",
        dbname,
...
fn au_039_does_not_fire_for_cross_breach_contamination() {
    ...
    let result = rule_au_039_wallet_identity(&RuleContext::new(&[wallet, person]), "scan", 0);
...
fn au_039_multiple_wallets_one_identity() {
    ...
    let result = rule_au_039_wallet_identity(
        &RuleContext::new(&[person.clone(), wallet1.clone(), wallet2.clone()]),
        "scan",
        0,
    );
...
fn au_039_email_anchor_when_person_unavailable() {
    ...
    let result = rule_au_039_wallet_identity(&RuleContext::new(&[email, wallet]), "scan", 0);
    assert_eq!(
        result.len(),
        1,
...
fn au_039_person_preferred_over_email_anchor() {
    ...
    let result = rule_au_039_wallet_identity(
        &RuleContext::new(&[person.clone(), email, wallet]),
        "scan",
        0,
    );
