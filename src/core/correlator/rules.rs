...
/// AU-044 — a cryptocurrency wallet matched the U.S. Treasury OFAC SDN list
/// (`ofac_sanctions`): the dominant verdict a crypto lead can carry. Unlike the
/// co-occurrence/exposure rules above, this is an authoritative designation —
/// any interaction with the wallet is a legal/criminal exposure — so it is
/// `Critical`. One firing per sanctioned wallet, anchored to the SDN party
/// Organisation when present.
pub(super) fn rule_au_044_sanctioned_wallet(
    entities: &[Entity],
    scan_id: &str,
    ts: u64,
) -> Vec<Correlation> {
    entities
        .iter()
        .filter(|e| e.kind == EntityKind::CryptoAddress && e.has_tag("ofac-sanctioned"))
        .map(|w| {
            let mut uids = vec![w.uid.clone()];
            if let Some(org) = entities
                .iter()
                .find(|e| e.kind == EntityKind::Organisation && e.has_tag("ofac-sanctioned"))
            {
                uids.push(org.uid.clone());
            }
            Correlation::new(
                "AU-044",
                "Cryptocurrency wallet on OFAC SDN sanctions list",
                Severity::Critical,
                format!(
                    "Wallet {} is on the U.S. Treasury OFAC SDN list — sanctioned party",
                    w.value
                ),
                uids,
                scan_id,
