//! MITRE ATT&CK® Enterprise catalogue — pure static data.
//!
//! Split out of `attack/mod.rs` so the ~4,100-line generated technique table
//! (regenerated from the pinned STIX bundle release, see [`super::ATTACK_VERSION`])
//! lives apart from the lookup functions and coverage logic that operate on it.
//! `mod.rs` re-exports [`TACTICS`] and [`ENTERPRISE`] so callers see no change.
