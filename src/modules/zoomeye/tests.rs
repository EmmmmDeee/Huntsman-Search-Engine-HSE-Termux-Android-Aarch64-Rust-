...
fn cdn_edge_ip_target_suppresses_coordinates_and_address() {
    // Regression: the old `skip_coords` gated Coordinates only — the Address
    // block built from the SAME untrusted match's city/country was emitted
    // unconditionally. A Cloudflare edge IP must suppress both, matching the
    // sibling IP-geo modules (onyphe included).
    ...
    let body = ZoomResp { matches: vec![m] };
    let target = Target::new(TargetKind::IpAddress, "104.16.0.1");
    let r = extract_entities(&body, &target, "104.16.0.1", "scan");
    assert!(
        !r.entities
            .iter()
...
#[test]
fn domain_query_suppresses_geo_only_for_the_cdn_edge_match() {
    // Regression: the old `skip_coords` was gated on `target.kind ==
    // IpAddress`, so it was ALWAYS false for a Domain-target query — every
    // match's geo went through unguarded, even one whose OWN `ip` field is a
    // CDN edge. A Domain-target dork can return several matches (a
    // Cloudflare-fronted domain shows different edge IPs across hosts), so
    // suppression must be evaluated per match, not once for the whole query.
    let cdn_match = serde_json::json!({
        "ip": "104.16.0.1",
        "geoinfo": {
            "country": {"code": "US", "names": {"en": "United States"}},
            "city": {"names": {"en": "San Francisco"}},
    ...
    let real_match = sample_match(); // ip 8.8.8.8, city Ashburn — not a CDN edge.
    let body = ZoomResp {
        matches: vec![cdn_match, real_match],
    };
    ...
    assert!(
        ...
        "the CDN-edge match's Address must be suppressed: {addresses:?}"
    );
    assert!(
        addresses.iter().any(|a| a.contains("Ashburn")),
        "the genuine match's Address must still surface: {addresses:?}"
    );
    ...
    assert_eq!(
        coords.len(),
        1,
        "only the non-CDN match's coordinates must surface: {coords:?}"
    );
}
