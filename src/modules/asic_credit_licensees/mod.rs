//! ASIC **Credit Licensee** register — keyless, free. A licensee name / ABN-ACN
//! seed → whether the entity holds an Australian credit licence, with its credit
//! licence number, ABN/ACN, recorded address and the exact supplied coordinates
//! ASIC publishes for the licensee.
//!
//! Source: the Australian Securities and Investments Commission publishes the
//! "ASIC – Credit Licensee Dataset" on `data.gov.au` (CKAN). Free, keyless,
...
//! [`crate::modules::asic_afs_licensees`]):
//!   1. `GET {ACTION_BASE}/package_show?id=asic-credit-licensee` to find the
...
//!
//! Why it matters for OSINT: a credit licence links a named organisation (or sole
//! trader) to a regulated consumer-credit relationship, its registered `ABN`/`ACN`
//! (a pivot into the whole corporate stack — `au_business_id`, `abn_lookup`,
//! `asic_director`, `opencorporates`), a registered business address, and — as for
//! the AFS register — the **exact latitude/longitude ASIC records for the
//! licensee**, parsed straight into `Coordinates` (no geocoding guess). The
//! register also carries a cross-link to the licensee's AFS licence number
//! (`CRED_LIC_AFSL_NUM`), kept in evidence. Matching is conservative and
//! whole-word: a row is a high-confidence finding only when its licensee name
//! contains every seed token as a whole word, or when its recorded ABN/ACN equals
//! the digits of an `AbnAcn` seed exactly. Loose full-text hits are surfaced as
//! sub-floor name-candidates carrying the full record in evidence (no omission).
...
pub(super) const SRC: &str = "asic_credit_licensees";

/// CKAN action-endpoint base for `data.gov.au` (note the extra `/data` path
/// segment vs. other portals, same as `asic_afs_licensees` / `agor`).
pub(super) const ACTION_BASE: &str = "https://data.gov.au/data/api/3/action";

/// CKAN dataset slug of the ASIC Credit Licensee register. Stable (it's the
...
/// module.
pub(super) const DATASET_ID: &str = "asic-credit-licensee";
...
// expansion floor so it pivots). Candidates (loose full-text hits) stay below the
// floor: surfaced but inert.
...
pub struct AsicCreditLicensees;

#[async_trait]
impl Module for AsicCreditLicensees {
    fn name(&self) -> &'static str {
        "asic_credit_licensees"
    }

    fn description(&self) -> &'static str {
        "ASIC Credit Licensee register (free, keyless) — licensee name / ABN → credit licence number, ABN/ACN, address, exact coordinates"
    }

    fn priority(&self) -> u8 {
        // Government / public-records band, alongside the other ASIC registries
        // (asic_afs_licensees 109).
        108
    }
    ...
    fn attack_techniques(&self) -> &'static [&'static str] {
        ...
        // (T1591.001) and the credit licence is a business/regulatory relationship
        // (T1591.002) — the same location+relationship posture as asic_afs_licensees.
        &["T1591.001", "T1591.002"]
    }
    ...
    fn max_timeout_ms(&self) -> u64 {
        ...
        // well above the 3s default (mirrors asic_afs_licensees / agor).
        12_000
    }
