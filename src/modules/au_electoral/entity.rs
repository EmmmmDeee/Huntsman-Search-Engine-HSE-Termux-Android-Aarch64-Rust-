...
pub(crate) fn build_electoral_entities(
    ...
    let (state, suburb, lat, lon, coord_conf) = if let Some(info) = division_centroid(division) {
        ...
        // Division not in offline table — try to infer state, or skip if unknown.
        let state = infer_state_from_division(division);
        (
            state,
    ...
    // Only emit when we have a real, valid AU state code — unknown divisions
    // without inferred state don't get an Address (avoid invalid "AU" tag).
    ...
    // Coordinates entity when we have an offline centroid and valid state.
    if let (Some(s), Some(lat), Some(lon)) = (state, lat, lon) {
