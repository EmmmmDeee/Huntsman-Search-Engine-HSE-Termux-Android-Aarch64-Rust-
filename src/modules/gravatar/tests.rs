...
fn extract_entry_emits_the_full_identity_graph() {
    ...
    // A linked account's URL must carry the SAME confidence as its own
    // Username sibling — both are the identical claim (this specific
    // linked account), so the URL scaling with `verified` independently
    // of the Username it's paired with would be a silent inconsistency.
    let javery_user = r
        .entities
        .iter()
        .find(|e| e.kind == EntityKind::Username && e.value == "javery")
        .expect("verified github username entity");
    let javery_url = r
        .entities
        .iter()
        .find(|e| e.kind == EntityKind::Url && e.value == "https://github.com/javery")
        .expect("verified github url entity");
    assert_eq!(javery_url.confidence, javery_user.confidence);
    assert_eq!(javery_url.confidence, confidence::HIGH);

    let mattd_user = r
        .entities
        .iter()
        .find(|e| e.kind == EntityKind::Username && e.value == "mattd")
        .expect("unverified twitter username entity");
    let mattd_url = r
        .entities
        .iter()
        .find(|e| e.kind == EntityKind::Url && e.value == "https://twitter.com/mattd")
        .expect("unverified twitter url entity");
    assert_eq!(mattd_url.confidence, mattd_user.confidence);
    assert_eq!(mattd_url.confidence, confidence::MEDIUM_HIGH);
}
...
fn extract_entry_surfaces_owner_emails_company_and_contact() {
    ...
    assert!(r.entities.iter().any(|e| e.kind == EntityKind::Url
...
fn real_profile_response_with_boolean_verified_parses_and_yields_entities() {
    ...
    assert!(
        r.entities.iter().any(|e| e.kind == EntityKind::Username
            && e.value == "beaulebens"
            && e.has_tag("twitter")
            && e.has_tag("verified")),
...
fn resolve_profile_builds_entities_from_a_real_profile() {
    let resp: GravatarResp = serde_json::from_str(r#"{"entry":[{"preferredUsername":"matt"}]}"#)
