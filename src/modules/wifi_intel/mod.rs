...
impl Module for WifiIntel {
    ...
    async fn process(&self, _target: &Target, ctx: &ModuleContext) -> Result<ModuleResult> {
        ...
        result.extend(aps.iter().map(|ap| {
            ...
            let mut e = Entity::new(EntityKind::MacAddress, &ap.bssid, confidence::VERY_HIGH_PLUSPLUS, &ctx.scan_id);
        ...
        for ap in aps.iter().take(MAX_BSSIDS) {
                ...
                && let (Some(lat), Some(lon)) = (detail.trilat, detail.trilong)
            {
                ...
                let mut e = Entity::new(EntityKind::Coordinates, &coords, confidence::HIGH_PLUSPLUS, &ctx.scan_id);
                e.tag("geoint");
                e.tag(crate::core::tags::WIFI_AP);
                e.tag("bssid-located");
                ...
                if addr_parts.len() >= 2 {
                    ...
                    let mut addr = Entity::new(EntityKind::Address, &addr_str, confidence::MEDIUM_PLUS, &ctx.scan_id);
                    addr.tag("geoint");
...
fn parse_aps(stdout: &[u8], scan_id: &str) -> ModuleResult {
    ...
    for ap in aps {
        let ssid = ap.ssid.as_deref().unwrap_or("<hidden>");
        let mut e = Entity::new(EntityKind::MacAddress, &ap.bssid, confidence::VERY_HIGH_PLUSPLUS, scan_id);
        e.tag(crate::core::tags::WIFI_AP);
        e.add_evidence(
            Evidence::new(SOURCE, format!("Wi-Fi AP: {ssid}"))
                .with_attr("ssid", ssid)
