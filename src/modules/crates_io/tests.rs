...
fn confirmed_username_entity_is_graded_at_cohort_canon() {
    let json = r#"{"user":{"id":1,"login":"alice"}}"#;
    let body: UserResp = serde_json::from_str(json).expect("should succeed");
    let ents = build_entities(&body, "scan-crates-001");
    let u = ents
        .iter()
    ...
    // HIGH_PLUSPLUS_PLUS (0.85), not EXPERT (0.88) — self-registration on a
    // package registry isn't authoritative/subject-supplied in EXPERT's sense.
...
fn crate_urls_dedup_sort_and_extract_github_owner() {
    ...
    assert!(
        !urls
            .iter()
            .any(|u| u.starts_with("ssh") || u.starts_with("ftp"))
    );
