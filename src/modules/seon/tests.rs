...
fn module_metadata() {
    ...
    // The email path's registrant extraction genuinely produces these.
    // `Url` stays declared too — even though it's confirmed practically dead
    // against the live API (see mod.rs's top doc comment), the deferred
    // phone path's `profile_url_entity` call is still a real literal
    // construction in this module's source, which `produces()` must match.
    for kind in [
        EntityKind::Person,
...
fn attack_techniques_drop_social_media_and_identify_roles() {
    ...
    for id in ["T1589.002", "T1589.003", "T1591.001", "T1591.002"] {
...
// ── Core: phone entity building (unchanged this cycle — see mod.rs's
// top doc comment: the phone path has the identical dead-extraction defect
// against the real API, deliberately deferred to a follow-up) ───────────
#[test]
...
// ── Pure entity-builder helpers (direct unit tests, phone-side only) ─
fn presence(registered: Option<bool>, url: Option<&str>) -> AccountPresence {
    AccountPresence {
        registered,
...
fn registered_accounts_keeps_only_registered_in_declared_order() {
    ...
    let fb = Some(presence(Some(true), Some("https://fb/ja")));
    let tw = Some(presence(Some(false), None)); // not registered → dropped
    let li = Some(presence(Some(true), None));
    ...
    assert_eq!(got[0].1.url.as_deref(), Some("https://fb/ja"));
}

#[test]
fn registered_accounts_empty_when_none_registered() {
    let a = Some(presence(Some(false), None));
    let b = Some(presence(None, None)); // registered is None, not true
