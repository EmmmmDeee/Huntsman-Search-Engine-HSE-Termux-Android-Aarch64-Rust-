...
fn deserializes_search_response() {
    ...
    let empty: SearchResp =
        serde_json::from_str(r#"{"objects":[],"total":0}"#).expect("should succeed");
...
fn full_record_yields_username_email_and_urls() {
    ...
    assert_eq!(
        attr("profile_url"),
        Some("https://www.npmjs.com/~kylo4kylo")
    );
...
#[test]
fn evidence_role_matches_the_field_the_person_actually_came_from() {
    // publisher/author/maintainer are three genuinely distinct npm fields
    // (self-declared vs. the account that ran `npm publish` vs. the
    // registry's current co-maintainer list) — regression for evidence
    // text/tags that used to hardcode "maintainer"/"author" regardless of
    // which field `person` was actually read from.
    let body = search(
        r#"{"objects":[{"package":{"name":"pkg",
                "publisher":{"username":"carol","url":"https://carol.dev"},
                "author":{"username":"dave","url":"https://dave.dev"},
                "maintainers":[
                    {"username":"alice","email":"alice@example.com"},
                    {"username":"bob","email":"bob@example.com"}
                ]}}],"total":1}"#,
    ...
    let carol = by_value(EntityKind::Username, "carol");
    assert!(carol.has_tag("co-publisher"), "carol came from `publisher`");
    assert!(!carol.has_tag("co-maintainer"));
    assert_eq!(carol.evidence[0].summary, "npm co-publisher of pkg");

    let dave = by_value(EntityKind::Username, "dave");
    assert!(dave.has_tag("co-author"), "dave came from `author`");
    assert!(!dave.has_tag("co-maintainer"));
    assert_eq!(dave.evidence[0].summary, "npm co-author of pkg");
    ...
    assert_eq!(
        alice_email.evidence[0].summary,
        "npm maintainer email (package pkg)"
    );
}
...
fn every_returned_package_is_emitted() {
    ...
    let objects: Vec<String> = (0..count)
        .map(|i| {
            format!(r#"{{"package":{{"name":"p{i}","links":{{"homepage":"https://h{i}.dev"}}}}}}"#)
        })
        .collect();
    let json = format!(r#"{{"objects":[{}],"total":{}}}"#, objects.join(","), count);
    let ents = build_entities(&search(&json), "alice", "s");
