...
impl Module for SeekNow {
    ...
    async fn process(&self, target: &Target, ctx: &ModuleContext) -> Result<ModuleResult> {
        ...
        if !ctx.cancel.is_cancelled() && see_know::budget_remaining() {
            ...
            // `dispatch_plan` returns `(per-endpoint (label, items), first_failure)`.
            // The first failure feeds `or_hard_failure` below — the single failure
            // path for this module — so there is no separate per-endpoint tally.
