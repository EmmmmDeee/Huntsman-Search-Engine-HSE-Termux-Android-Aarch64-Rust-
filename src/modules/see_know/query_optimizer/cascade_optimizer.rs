    ...
    pub fn should_cascade(
        ...
        let (should_cascade, reasoning) = match pivot_tier {
            PivotTier::Tier1 => {
                ...
                if budget_remaining >= 50 && time_remaining_secs >= 30 {
                    (
                        true,
                        format!(
                            "Tier-1 pivot: cascade (budget: {budget_remaining}, time: {time_remaining_secs}s)"
                        ),
                    )
                } else {
                    (
                        false,
                        "Tier-1 pivot: insufficient budget or time".to_string(),
                    )
                }
            }
            PivotTier::Tier2 => {
                // Cascade only if ROI positive and threshold met
                if pivot_roi > threshold
                    && budget_remaining > (threshold as u32)
                    && time_remaining_secs >= 30
                {
                    (
                        true,
                        format!(
                            "Tier-2 pivot: ROI {pivot_roi:.1} > threshold {threshold:.1}, cascade"
                        ),
                    )
                } else {
                    (
                        false,
                        format!(
                            "Tier-2 pivot: ROI {pivot_roi:.1} <= threshold {threshold:.1}, skip"
                        ),
                    )
                }
            }
            PivotTier::Tier3 => {
                ...
                (
                    false,
                    "Tier-3 pivot: low value, skip by default".to_string(),
                )
            }
        };
...
mod tests {
    ...
    fn test_pivot_tier_classification() {
        ...
        assert_eq!(
            optimizer.classify_pivot_tier("coordinates"),
            PivotTier::Tier3
        );
    }
