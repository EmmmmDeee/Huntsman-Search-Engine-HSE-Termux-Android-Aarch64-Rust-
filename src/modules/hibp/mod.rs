    ...
    async fn process(&self, target: &Target, ctx: &ModuleContext) -> Result<ModuleResult> {
        ...
        match target.kind {
            TargetKind::Email => {
                ...
                // same Email entity. `key`/`tried` thread the in-scan key cascade
                // through both calls, so a key exhausted mid-way rotates rather
                // than failing the rest of this process() call.
