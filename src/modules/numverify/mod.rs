...
fn build_entities(r: &NvResp, scan_id: &str) -> Vec<Entity> {
    ...
    let mut e = Entity::new(EntityKind::Address, &place, confidence::MEDIUM_HIGH, scan_id);
    e.tag(SRC);
    e.tag("geo-hint");
    e.tag("phone-region");
    ...
    if let Some(carrier) = r.carrier.as_deref().map(str::trim).filter(|c| c.len() >= 2) {
        let mut oe = Entity::new(EntityKind::Organisation, carrier, confidence::MEDIUM_PLUS, scan_id);
        oe.tag(SRC);
        oe.tag("carrier");
