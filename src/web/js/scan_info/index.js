  ...
  v.innerHTML = `
    ...
    <h2>${esc(scan.target?.value||id)}
        ...
        <div class="pull-right">
          ...
          <a class="btn btn-warning btn-sm" href="${API.batchUrl(id)}" download data-download title="⚠ OPERATOR ONLY — every email, username, phone, domain, IP and name this scan found, as one plaintext list per breach provider (OathNet, SeekNow, …) in the syntax its search page accepts. Paste by hand; no API key needed. Same as `hse batch --scan-id`."><i class="glyphicon glyphicon-list"></i>&nbsp;Bulk queries (operator)</a>
