...
| Stealer-row parse now honours the forced format (completes the override) — the upload handler threaded `?format=` into `entities_from_upload` but not the separate `stealer_rows_from_upload`, which re-ran content detection. A forced `?format=stealerlogs` upload whose body detection MISSES (a `Credentials:` list with no `Log Id:` line — `looks_like_stealerlogs` needs all three markers, `parse_stealerlogs` does not) kept its entities but silently dropped its paired rows; the inverse (force a non-stealer format on a stealer body) emitted rows the operator overrode. Found by a disconfirming probe of the override's OWN diff. `stealer_rows_from_upload` now takes `forced` and honours it identically; `None` (auto-detect / CLI) is the unchanged content check. Regression: a unit test (detection rejects the no-Log-Id body, parser still recovers the row, forced recovers it, auto-detect does not, force-away emits nothing) and an API test (POST `?format=stealerlogs` → GET stealer-rows). **Falsified**: revert to ignore `forced` → both fail (rows `[]`); restored byte-identical; gate --quick green 15/15. **Proven LIVE end-to-end** against a running `hse serve` on loopback: the forced upload returned `format:stealerlogs` + `entity_count:2` and persisted the paired row (login `jordanavery@gmail.com`, password `Hunter2pass`, `pwned_at` set, `log_id` null) retrievable from the real `/scans/{id}/stealer-rows` endpoint, reproduced on a second fresh scan id; the identical body with NO override is rejected `400 no verifiable entities`, so the rows exist only via the forced path | `a1e96042` | on PR #624, CI re-triggered on this head (being watched to green) |
...
| Live end-to-end verification (the stealer-row forced-format fix, `a1e96042`) — TEST PASS treated as insufficient, per the operator's live-proof directive | rebuilt the binary, ran a real `hse serve` on 127.0.0.1, and drove the actual HTTP path: `POST /api/v1/scans/import?format=stealerlogs` with a no-`Log Id:` body → 200 `format:stealerlogs`, then `GET /scans/{id}/stealer-rows` returned the paired row from the real DB (not mocked/cached — a fresh scan id each run, reproduced twice with distinct ids). Disconfirming control: the identical body with no `?format=` is rejected `400 no verifiable entities`, proving the rows are produced by the forced path and not a fallback. Server torn down cleanly afterwards |
