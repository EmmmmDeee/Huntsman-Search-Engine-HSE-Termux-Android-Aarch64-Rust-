...
| ▸ | 2026-08-26 | #467 | Close an unchecksummed ABN/ACN validation gap in `asic_persons`/`asic_banned_orgs`/`asic_business_names` (digit-count-only accept → route through the existing `util::abn::is_valid_abn`/`is_valid_acn`), and hoist their triplicated null-filtered CKAN field wrapper into `util::ckan::field`. | Correctness on a live/hot data path (SELECT's top-ranked category): all three ASIC keyless-register modules accepted any 9/11-digit register field as an ACN/ABN pivot with no checksum, though `util::abn`'s own module doc names "registry fields" as its intended caller and `asic_persons` already imports `util::abn` for `looks_like_company`. `name_tokens`/`record_name_matches`/`humanise_name` were surveyed as similar but deliberately left alone: `sanctions_ofac::parse`'s own doc comments document "each module owns its parsing helpers" for that family, so consolidating them would relitigate a documented decision, not fix a defect. | Red (captured via `git stash` on just the implementation, tests kept): 3 new `checksum_invalid_*` tests FAILED. Green after the fix: full `scripts/gate.sh` 8/8 executable checks PASS (fmt, check, clippy -D warnings, rustdoc lints, 6506/6506 lib tests, doctests, doc coverage held at 1064, install.sh syntax); PR #467 CI green 7/7 including MSRV 1.88 and the aarch64-linux-android cross-build (unavailable on this host). Open/draft, `mergeable_state: clean`, no reviews yet as of this row. Ledger gap 2026-08-03→2026-08-26 (PRs #335–#466, 140+ commits) is left unreconstructed for the same INV-1 reason as the #331–#333 gap above: this repo saw substantial ordinary iterative development across many sessions in that window without formal charter-cycle rows, and recording it second-hand here would be fabrication. | Consolidating `name_tokens`/`record_name_matches`/`humanise_name` across the ASIC+OFAC modules: rejected — see leverage rationale. Further collapsing each module's 3-line `ckan_query` wrapper: rejected — already a thin call into the shared `util::ckan::validated_result`; a wrapper-of-a-wrapper would trade a few lines for less-obvious per-module resource-id plumbing. |
