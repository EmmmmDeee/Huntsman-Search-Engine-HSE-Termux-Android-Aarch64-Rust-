...
| ~~`scripts/architecture_audit.py`~~ | 0 | **Closed 2026-08-28**, retroactive to this snapshot: ported to `src/bin/architecture_audit/` (a `[[bin]]` target, byte-identity-verified against the Python original — both JSON and text report modes — against a real 188-module live `hse serve` capture before removal). The original's own reasoning for staying Python ("deliberately external to the binary it inspects") conflated process separation, which a second `[[bin]]` target preserves exactly as well, with language choice, which it does not actually require. Row kept for the audit trail rather than deleted; no longer a genuine exception. |
...
| ~~`scripts/pack_monolith.py`~~ | 0 | **Closed 2026-08-29**, retroactive to this snapshot: ported to `src/bin/pack_monolith/` (a `[[bin]]` target). The record format is byte-compatible with the Python original — the Python `--verify` reconstructs the byte-exact tree from the Rust packer's output before removal, and the bin carries its own round-trip test (text with/without trailing newline, empty, CRLF, NUL-binary, invalid-UTF-8, and a payload whose content mimics a metablock key). Same reasoning as the two ported rows above: process separation ("a meta-tool about the repository, not part of it") is preserved exactly by a second `[[bin]]` target, while the language choice it was conflated with is not actually required. Row kept for the audit trail; no longer a genuine exception. |
...
| ~~`.agent/merge_state.py`~~ | 0 | **Corrected 2026-08-28**, retroactive to this snapshot: the file does not exist in this tree (`git ls-files .agent/` lists only `state.json`) — this row was stale bookkeeping, not a live exception, discovered while surveying this table for the next migration candidate. Row kept rather than deleted for the same audit-trail reason as the two closed rows above. |
