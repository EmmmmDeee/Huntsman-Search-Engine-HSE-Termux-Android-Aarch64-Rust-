...
| `scripts/gen_oui.py` | 1 | Regenerates `src/util/oui/ieee.bin`, a packed binary blob, from the IEEE MAC-vendor registry. Its own header documents why the *output* is a hand-designed binary format rather than generated Rust source (avoids multi-MB compile-time cost and per-string relocations on the Android target) — the generator itself is a one-off, occasionally-run offline tool, not runtime code. |
...
| Binary data blob | `src/util/oui/ieee.bin` (generated by 5a's `gen_oui.py`, consumed via `include_bytes!` — the *consumer* is Rust) | 1 |
