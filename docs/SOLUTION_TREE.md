...
- **2026-06-24** — **Cycle 37 (P→S): SOL-TRACKING-PIVOT — tracking ID pivot mechanism verified complete.**
  ...
  - *SOL-TRACKING-PIVOT-C — gap forwarded:* For future consideration — add a priority boost for `Url` entities tagged `social-profile` so they rank above generic domains in the expansion candidate heap, ensuring social-profile crawls happen early in the round rather than being pre-empted by higher-volume domain/IP targets.
