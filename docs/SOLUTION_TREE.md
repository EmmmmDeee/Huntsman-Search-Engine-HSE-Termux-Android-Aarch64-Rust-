...
| SOL-MITRE-DIVERSITY | T2.39 (technique-diversity corroboration) | `[~]` |
| SOL-CACHE-TEST-ISOLATION | T2.40 (serialise global-cache tests) | `[x]` |
| SOL-TARGETMATCH-TOKEN | T2.41 (token-boundary row attribution) | `[x]` |
| SOL-NETLAS-EMAIL-PROVENANCE | T2.42 (provenance-tiered email confidence) | `[x]` |
...
- **2026-07-10** — **SOL-MITRE-DIVERSITY `[ ]`→`[~]` (advances T2.39): ATT&CK technique-diversity as source independence — the *duality* mapping.** `source_family` and the ATT&CK technique map are two frames of the same corroboration physics; source independence is INVISIBLE in the coarse `infra` family bucket but MANIFEST in the ATT&CK frame (T1590 vs T1596.005 vs T1591.001). Added `core::attack::entity_techniques`, a `Correlation.techniques` field (union of child `attack:<ID>` provenance, populated at `rank_and_sort`), a diversity tie-break (after rank+severity — primary order unchanged), and a dossier MITRE line per correlation. ATT&CK is now per-CONCLUSION provenance, a measure of independence. **⚑ built on** SOL-MITRE-COVERAGE's parent/sub rollup. **`[~]` not `[x]`:** the deeper reward (a `min(distinct_sources, distinct_techniques)` term in the `c_effective` noisy-OR + an AU-112 multi-technique rule) is the open follow-up. +1 test. Paired: `PROBLEM_TREE` T2.39 §8 — same commit.
