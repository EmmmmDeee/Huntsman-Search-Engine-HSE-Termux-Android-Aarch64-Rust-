...
| `proxies` | `cli/proxies.rs` | Proxy retriever: `refresh` harvests + validates + persists free proxies (`util::proxy`), `list` shows them. Scans route through them with `HUNTSMAN_PROXY=auto`. |
