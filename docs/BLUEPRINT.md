...
| 2 | **Graph engine / entity-linking** | `ModuleGraph` (dispatch + richness), the correlator's 38 rules (incl. **graph-aware AU-031/AU-032/AU-034** that walk the edges, **AU-037 shared-web-tracker operator clustering**, and **AU-038 pattern-of-life location exposure**), **and first-class typed `Relation` edges** (structural + lineage + geo co-location + DNS resolution + WHOIS registration + image similarity + stealer co-occurrence); GEXF export + D3 force graph | `core/dependency.rs`, `core/correlator/`, `core/relation.rs`, `core/gexf.rs` |
