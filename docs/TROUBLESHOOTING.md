...
> shipped behaviour. Flags referenced below — `--cascade-depth`,
> `--no-cascade`, `--fast-only`, `--retry`, `--no-cache`, `hse scan --dry-run`
> and the "Plan tier" detection — do not exist in the current `hse` CLI and
> will be rejected as unrecognised arguments. Run `hse scan --help` /
> `hse doctor` to see what is actually implemented today; do not paste
> commands from this page verbatim.
