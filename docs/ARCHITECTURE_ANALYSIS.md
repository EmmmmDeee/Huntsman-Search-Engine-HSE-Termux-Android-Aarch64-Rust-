...
| 4 | Collection breadth — new keyless modules per the one-file module contract, each with fixture proof | Intelligence quality | High (recall) | **in progress** — `security_txt` (RFC 9116 published security contacts) shipped as the first; CommonCrawl index / DNSBL remain (roadmap §4.A of `ULTIMATE_ARCHITECTURE.md`). The highest-value remaining lever now that consolidation is done. |
